...
class ManagerHome extends CI_Controller {
    ...
    public function index(){
        ...
        if($data['user']['sesilogin']){
            if($data['user']['sesilogin']['id_position'] == 3){
                $data['title'] = "PM | Home";
                $data['dashboard'] = "Dashboard PM";
                $data['trip'] = $this->db->get_where('travels',['id_user'=>$data['user']['sesilogin']['id_user']])->result_array();
                $this->load->view('templates/header',$data);
                $this->load->view('Manager/V_manager',$data);
    ...
    public function perjalanan(){
        ...
        if($data['user']['sesilogin']){
            if($data['user']['sesilogin']['id_position'] == 3){
                $data['title'] = "PM | Tambah Data Perjalanan";
                $data['dashboard'] = "Dashboard PM";
                $this->load->view('templates/header',$data);
                $this->load->view('Manager/Vm_dataperjalanan',$data);
                $this->load->view('templates/footer');
            }else{
                $data['heading'] = "Halaman tidak dapat diakses";
                $data['message'] = " Mohon maaf akses ditolak";
    ...
    public function addperjalanan(){
        ...
        if($cek){
            ...
            redirect('managerhome/perjalanan');
        }else{
    ...
    public function end_trip($idtrv){
        ...
        $cek = $this->db->get_where('users',["id_user"=>$sesi['sesilogin']['id_user']])->row_array();
        if($cek){
            $this->m_user->endtravelm($sesi['sesilogin']['id_user'],$idtrv);
            redirect('managerhome/history');
        }else{
    ...
    public function detail_history($id){
        ...
        if($data['user']['sesilogin']){
            if($data['user']['sesilogin']['id_position'] == 3){
                $data['title'] = "PM | Detail History Perjalanan";
                $data['dashboard'] = "Dashboard PM";
                ...
                $this->load->view('templates/header',$data);
                $this->load->view('Manager/Vm_detailhistory',$data);
                $this->load->view('templates/footer');
            }else{
                $data['heading'] = "Halaman tidak dapat diakses";
                $data['message'] = " Mohon maaf akses ditolak";
    ...
    public function approval(){
        ...
        if($data['user']['sesilogin']){
            if($data['user']['sesilogin']['id_position'] == 3){
                $data['title'] = "PM | Approval";
                $data['dashboard'] = "Dashboard PM";
                $data['sweeper'] = $this->m_user->getapproval(3,1);
                $this->load->view('templates/header',$data);
                $this->load->view('Manager/Vm_approval',$data);
                $this->load->view('templates/footer');
            }else{
                $data['heading'] = "Halaman tidak dapat diakses";
                $data['message'] = " Mohon maaf akses ditolak";
    ...
    public function detail_travel($id){
        ...
        if($data['user']['sesilogin']){
            if($data['user']['sesilogin']['id_position'] == 3){
                $data['title'] = "PM | Detail Travel";
                $data['dashboard'] = "Dashboard PM";
                $data['trv'] = $this->db->get_where('travels',['id_travel'=>$id])->row_array();
                $data['hist'] = $this->db->get_where('detail_travels',['id_travel'=>$id])->result_array();
                $this->load->view('templates/header',$data);
                $this->load->view('Manager/Vm_detailtravel',$data);
                $this->load->view('templates/footer');
            }else{
                $data['heading'] = "Halaman tidak dapat diakses";
                $data['message'] = " Mohon maaf akses ditolak";
    ...
    public function approve($id){
        ...
        $data = [
            'status' => 2,
            'keterangan'=>"Reviewed by ".$sesi['sesilogin']['fullname']." Tanggal ".date('d-m-yy')
        ];
        $this->db->where('id_travel',$id);
        $this->db->update('travels',$data);
        redirect('managerhome/approval');
    }
    ...
    public function user(){
        ...
        if($data['user']['sesilogin']){
            if($data['user']['sesilogin']['id_position'] == 3){
                $data['title'] = "PM | Tambah Sweeper";
                $data['dashboard'] = "Dashboard PM";
                $data['user'] = $this->m_user->getuser(4);
                $this->load->view('templates/header',$data);
                $this->load->view('Manager/Vm_user',$data);
                $this->load->view('templates/footer');
            }else{
                $data['heading'] = "Halaman tidak dapat diakses";
                $data['message'] = " Mohon maaf akses ditolak";
    ...
    public function addsweeper(){
        ...
        if($this->form_validation->run() == false){
            ...
            redirect('managerhome/user','refresh');
        }
    }
    ...
    public function invoice($id){
        ...
        $html = $this->load->view('Manager/Invoice',$data,true);
    ...
    public function save_changes($idtrv){
        ...
        redirect('managerhome/detail_history/'.$idtrv);
    }
    public function edit_detail($id){
        ...
        redirect('managerhome/detail_history/'.$idtrv['id_travel']);
    }
