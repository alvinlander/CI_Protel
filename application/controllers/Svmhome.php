...
class SvmHome extends CI_Controller {
    ...
    public function index(){
        ...
        if($data['user']['sesilogin']){
            if($data['user']['sesilogin']['id_position'] == 2){
                $data['title'] = "SPM | Home";
                $data['dashboard'] = "Dashboard SPM";
                $data['trip'] = $this->db->get_where('travels',['id_user'=>$data['user']['sesilogin']['id_user']])->result_array();
                $this->load->view('templates/header',$data);
                $this->load->view('Svm/V_svm',$data);
    ...
    public function perjalanan(){
        ...
        if($data['user']['sesilogin']){
            if($data['user']['sesilogin']['id_position'] == 2){
                $data['title'] = "SPM | Tambah Data Perjalanan";
                $data['dashboard'] = "Dashboard SPM";
                $this->load->view('templates/header',$data);
                $this->load->view('Svm/Vsvm_dataperjalanan',$data);
                $this->load->view('templates/footer');
            }else{
                $data['heading'] = "Halaman tidak dapat diakses";
                $data['message'] = " Mohon maaf akses ditolak";
    ...
    public function detail_history($id){
        ...
        if($data['user']['sesilogin']){
            if($data['user']['sesilogin']['id_position'] == 2){
                $data['title'] = "SPM | Detail History Perjalanan";
                $data['dashboard'] = "Dashboard SPM";
                ...
                $this->load->view('templates/header',$data);
                $this->load->view('Svm/Vsvm_detailhistory',$data);
                $this->load->view('templates/footer');
            }else{
                $data['heading'] = "Halaman tidak dapat diakses";
                $data['message'] = " Mohon maaf akses ditolak";
    ...
    public function approval(){
        ...
        if($data['user']['sesilogin']){
            if($data['user']['sesilogin']['id_position'] == 2){
                $data['title'] = "SPM | Approval";
                $data['dashboard'] = "Dashboard SPM";
                $data['sweeper'] = $this->m_user->getapproval(2,2);
                $this->load->view('templates/header',$data);
                $this->load->view('Svm/Vsvm_approval',$data);
                $this->load->view('templates/footer');
            }else{
                $data['heading'] = "Halaman tidak dapat diakses";
                $data['message'] = " Mohon maaf akses ditolak";
    ...
    public function detail_travel($id){
        ...
        if($data['user']['sesilogin']){
            if($data['user']['sesilogin']['id_position'] == 2){
                $data['title'] = "SPM | Detail Travel";
                $data['dashboard'] = "Dashboard SPM";
                $data['trv'] = $this->db->get_where('travels',['id_travel'=>$id])->row_array();
                $data['hist'] = $this->db->get_where('detail_travels',['id_travel'=>$id])->result_array();
                $this->load->view('templates/header',$data);
                $this->load->view('Svm/Vsvm_detailtravel',$data);
                $this->load->view('templates/footer');
            }else{
                $data['heading'] = "Halaman tidak dapat diakses";
                $data['message'] = " Mohon maaf akses ditolak";
    ...
    public function approve($id){
        ...
        } else if($posisi['id_position'] == 3){
                $data = [
                    'status' => 3,
                    'keterangan'=>$cek['keterangan'].","."Reviewed by ".$sesi['sesilogin']['fullname']." Tanggal ".date('d-m-yy')
                ];
                $this->db->where('id_travel',$id);
                $this->db->update('travels',$data);
        }
        redirect('svmhome/approval');
    }
    ...
    public function user(){
        ...
        if($data['user']['sesilogin']){
            if($data['user']['sesilogin']['id_position'] == 2){
                $data['title'] = "SPM | Tambah Manager";
                $data['dashboard'] = "Dashboard SPM";
                $data['user'] = $this->m_user->getuser(3);
                $this->load->view('templates/header',$data);
                $this->load->view('Svm/Vsvm_user',$data);
                $this->load->view('templates/footer');
            }else{
                $data['heading'] = "Halaman tidak dapat diakses";
                $data['message'] = " Mohon maaf akses ditolak";
    ...
    public function invoice($id){
        ...
        $html = $this->load->view('Svm/Invoice',$data,true);
    ...
    public function save_changes($idtrv){
        ...
        redirect('svmhome/detail_history/'.$idtrv);
    }
    public function edit_detail($id){
        ...
        redirect('svmhome/detail_history/'.$idtrv['id_travel']);
    }
